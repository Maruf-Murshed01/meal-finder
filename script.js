...
function displayMealData(mealLists) {
  if (mealLists === null) {
    foodContainer.innerHTML = `<h1 class="not-found-result">sorry! search item not found</h1>`;
  } else {
    mealLists.forEach((meal) => {
      ...
      <h4 class="mt-2 food_name">${strMeal}</h4>
      </a>
        `;
...
btn.addEventListener("click", () => {
  if(input.value.length !== 0 && input.value !== " "){
    ...
    foodContainer.innerHTML = `<h1 class="blank-result">Please Write Something</h1>`
  }
...
function getMealDetails(makedDiv, mealId) {
  makedDiv.addEventListener("click", () => {
...
function showRecipeDetails(mealsData) {
  if (mealsData === null) {
    ...
    <img src="${strMealThumb}"class="img_meal_recipe" alt="food image"/>
          <h1 class="mt-3 str_meal">${strMeal}</h1>
